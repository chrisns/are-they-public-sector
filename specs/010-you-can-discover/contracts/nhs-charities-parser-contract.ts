...
describe('NHSCharitiesParser Contract', () => {
  ...
  describe('discoverApiUrl()', () => {
    it('should extract API URL from NHS Charities webpage', async () => {
      // Act
      const apiUrl = await (parser as any).discoverApiUrl();
    ...
    it('should extract map ID from JavaScript', async () => {
      ...
      // Act
      const mapId = (parser as any).extractMapId(mockHtml);
    ...
    it('should handle missing map ID', async () => {
      ...
      // Act & Assert
      expect(() => (parser as any).extractMapId(mockHtml)).toThrow('Map ID not found');
    });
  });
  ...
  describe('fetchApiData()', () => {
    it('should fetch JSON from Storepoint API', async () => {
      ...
      // Act
      const data = await (parser as any).fetchApiData(apiUrl);
    ...
    it('should handle API errors gracefully', async () => {
      ...
      jest.spyOn(parser as any, 'fetchApiData').mockRejectedValue(
        new Error('API error')
      );
  ...
  describe('filterEnglandWales()', () => {
    it('should filter charities to England and Wales only', () => {
      ...
      // Act
      const filtered = (parser as any).filterEnglandWales(mockCharities);
  ...
  describe('retry logic', () => {
    it('should retry on network failure', async () => {
      // Arrange
      const fetchSpy = jest.spyOn(parser as any, 'fetchHTML')
        .mockRejectedValueOnce(new Error('Network error'))
        .mockResolvedValueOnce('<html>...</html>');

      // Act
      await (parser as any).fetchWithRetry();
    ...
    it('should fail after max retries', async () => {
      // Arrange
      jest.spyOn(parser as any, 'fetchHTML')
        .mockRejectedValue(new Error('Network error'));

      // Act & Assert
      await expect((parser as any).fetchWithRetry()).rejects.toThrow('Failed after retries');
    });
  });
});