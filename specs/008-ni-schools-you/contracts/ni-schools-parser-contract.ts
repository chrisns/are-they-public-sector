...
describe('NISchoolsParser Contract', () => {
  ...
  describe('parse()', () => {
    ...
    it('should handle network timeouts appropriately', async () => {
      ...
      jest.spyOn(parser as any, 'fetchPage').mockRejectedValue(timeoutError);
    ...
    it('should use correct form parameters for export', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          'ctl00$ContentPlaceHolder1$instStatus': '0',
  ...
  describe('error handling', () => {
    it('should provide clear error messages for common failures', async () => {
      const scenarios = [
        {
          mock: () => jest.spyOn(parser as any, 'fetchPage').mockRejectedValue(new Error('ECONNREFUSED')),
          expectedError: /connection refused|service unavailable/i
        },
        {
        ...
        {
          mock: () => {
            jest.spyOn(parser as any, 'fetchPage').mockResolvedValue('<input id="__VIEWSTATE" value="test" />');
