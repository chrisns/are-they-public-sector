...
describe('EnglishCourtsParser Contract', () => {
  ...
  describe('parse()', () => {
    it('should fetch and parse courts data from the CSV URL', async () => {
      // Arrange
      const expectedUrl = 'https://factprod.blob.core.windows.net/csv/courts-and-tribunals-data.csv';
    ...
    it('should fail fast on network error', async () => {
      // Arrange
      jest.spyOn(parser as any, 'fetchCSV').mockRejectedValue(
    ...
    it('should fail fast on invalid CSV format', async () => {
      // Arrange
      jest.spyOn(parser as any, 'fetchCSV').mockResolvedValue(
